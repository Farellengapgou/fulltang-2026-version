...
export function SocialChargesCalculator() {
  ...
  const calculations = React.useMemo(() => {
    ...
    taxRates.forEach((tax) => {
      const baseAmount =
        tax.base === "masseSalariale"
          ? chargesData.masseSalariale
          : chargesData.effectif;
      const amount =
        tax.type === "percentage"
    ...
    results.totalCnps =
      (results.cnps_salarie || 0) + (results.cnps_employeur || 0);
  ...
  const saveEditingTax = () => {
    setTaxRates((prev) =>
      prev.map((tax) => (tax.id === editingTax.id ? editingTax : tax))
    );
    setEditingTax(null);
  };
  ...
  const deleteTax = (taxId) => {
    setTaxRates((prev) =>
      prev.filter((tax) => tax.id !== taxId && tax.mandatory)
    );
  };
      ...
      <div className="max-w-7xl mx-auto p-6 bg-gradient-to-br from-gray-50 to-blue-50 min-h-screen">
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-gray-600 mt-1">
                  Calculs automatiques et gestion des échéances
                </p>
              </div>
            </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-gray-500">Effectif total déclaré</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          {[
            ...
            <div
              key={index}
              className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6 relative overflow-hidden group hover:shadow-xl transition-all duration-300"
            >
              <div
                className={`absolute top-0 right-0 w-20 h-20 bg-gradient-to-br ${item.color} rounded-full -mr-10 -mt-10 opacity-10 group-hover:opacity-20 transition-opacity`}
              ></div>
              <div className="relative">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-2xl">{item.icon}</span>
                  <div
                    className={`px-3 py-1 bg-gradient-to-r ${item.color} text-white rounded-full text-xs font-semibold`}
                  >
                    CALCULÉ
                  </div>
                </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8 mb-8">
          ...
          <div className="space-y-4">
            {taxRates.map((tax) => (
              <div
                key={tax.id}
                className={`border-2 rounded-xl p-6 transition-all duration-200 ${getColorClasses(
                  tax.color
                )}`}
              >
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center space-x-4 mt-3">
                        <span className="text-sm font-medium">
                          Base:{" "}
                          {tax.base === "masseSalariale"
                            ? formatCurrency(chargesData.masseSalariale)
                            : `${chargesData.effectif} employés`}
                        </span>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center space-x-2 text-sm text-orange-600">
              ...
              {
                deadlines.filter((d) => getDaysUntilDeadline(d.deadline) <= 15)
                  .length
              }{" "}
              échéance(s) proche(s)
            </div>
          ...
          <div className="space-y-6">
            {deadlines.map((deadline) => {
              ...
              return (
                <div
                  key={deadline.id}
                  className="border-2 border-gray-200 rounded-xl p-6 hover:shadow-lg transition-all duration-200"
                >
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      <div className="flex items-center space-x-3 mb-2">
                        ...
                        <span
                          className={`px-3 py-1 rounded-full text-sm font-semibold border ${priorityClass}`}
                        >
                          {daysLeft > 0
            ...
            <div className="bg-white rounded-2xl p-8 max-w-md w-full mx-4">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <select
                      value={newTax.type}
                      onChange={(e) =>
                        setNewTax((prev) => ({ ...prev, type: e.target.value }))
                      }
                      className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500"
                    >
                ...
                <select
                  value={newTax.base}
                  onChange={(e) =>
                    setNewTax((prev) => ({ ...prev, base: e.target.value }))
                  }
                  className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500"
                >
                  <option value="masseSalariale">Masse salariale</option>
