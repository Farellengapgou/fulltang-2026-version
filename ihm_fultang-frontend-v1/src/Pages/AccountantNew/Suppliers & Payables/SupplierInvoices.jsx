    ...
    const loadInvoices = useCallback(async () => {
        ...
        try {
            ...
            if (!data || data.length === 0) {
                 setInvoices([
                     { id: 1, number: 'INV-SUP-001', supplierName: 'CAMEG', date: '2024-01-10', totalAmount: 2500000, status: 'PAID' },
                     { id: 2, number: 'INV-SUP-002', supplierName: 'Laborex', date: '2024-01-12', totalAmount: 450000, status: 'PENDING' },
                     { id: 3, number: 'INV-SUP-003', supplierName: 'Eneo', date: '2024-01-05', totalAmount: 150000, status: 'OVERDUE' },
                 ]);
            } else {
                setInvoices(data);
            }
            ...
            setErrorMessage("Impossible de charger les factures fournisseurs.");
        } finally {
            setIsLoading(false);
        }
    }, []);
        ...
        <AccountantDashBoard linkList={FinancialAccountantNavLink} requiredRole={"Accountant"}>
            ...
            <div className="mx-auto p-12">
                <div className="flex justify-between items-center mb-6">
                    <div>
                        <h1 className="text-2xl font-bold text-gray-800">Factures Fournisseurs</h1>
                        <p className="text-gray-600 mt-1">Suivi des achats et factures reçues</p>
                    </div>
                    <div className="flex gap-3">
                        <button onClick={loadInvoices} className="flex items-center px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transaction-all">
                            ...
                            {filteredInvoices.length > 0 ? (
                                filteredInvoices.map((invoice) => (
                                    <tr key={invoice.id} className="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{invoice.supplierName || 'Fournisseur Inconnu'}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900 text-right">{formatCurrency(invoice.totalAmount || 0)}</td>
