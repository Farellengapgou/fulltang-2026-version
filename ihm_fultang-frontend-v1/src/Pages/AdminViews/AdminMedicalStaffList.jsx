...
export function AdminMedicalStaffList() {
    ...
    const filteredMedicalStaffs = medicalStaffList.filter((medicalStaff) => {
        const medicalStaffFullName = `${medicalStaff.first_name} ${medicalStaff.last_name}`.toLowerCase();
        const medicalStaffEmail = medicalStaff.email?.toLowerCase() || "";
        const matchesSearch = searchTerm === "" ||
            medicalStaffFullName.includes(searchTerm.toLowerCase()) ||
            medicalStaffEmail.includes(searchTerm.toLowerCase());

        const matchesRole = roleFilter === "All" || medicalStaff.role === roleFilter;

        return matchesRole && matchesSearch;
    });

    const displayMedicalStaffs = (searchTerm || roleFilter !== "All") ? filteredMedicalStaffs : medicalStaffList;
    const uniqueRoles = ["All", ...new Set(medicalStaffList.map(staff => staff.role))];




    function calculateNumberOfSlide() {
        const totalItems = (searchTerm || roleFilter != "All") ? filteredMedicalStaffs.length : numberOfMedicalStaff;
                    ...
                    <div className="flex mr-5">
                        <div className="mr-4 font-bold">
                            ...
                            <select
                                ...
                                className="ml-2 w-30 h-10 text-white bg-gray-300 px-2 border-gray-200 rounded-lg"
                            >
                                {uniqueRoles.map((role) => (
                                    <option key={role} value={role}>
                                        {role === "Labtech" || role === "LabTech" ? "Laboratory Assistant" : role === "MaterialAccountant" ? "Comptable Matière" : role}
                                    </option>
                                ))}
                            </select>
                        </div>
                            ...
                            (
                                    ...
                                    <table className="w-full border-separate border-spacing-y-2">
                                        <thead>
                                            <tr className="bg-gradient-to-l from-primary-start to-primary-end">
                                                ...
                                                <th className="text-center text-white p-4 text-xl font-bold">Last Name</th>
                                        ...
                                        <tbody>
                                            {displayMedicalStaffs.map((medicalStaff, index) => (
                                                <tr key={medicalStaff.id || index} className="bg-gray-100">
                                                    ...
                                                    <td className="p-4 text-center text-md">{medicalStaff.role === "Labtech" || medicalStaff.role === "LabTech" ? "Laboratory Assistant" : medicalStaff.role === "MaterialAccountant" ? "Comptable Matière" : medicalStaff.role}</td>
                                                    <td className="p-4 relative rounded-r-lg">
                                                        <div className="w-full items-center justify-center flex gap-6">
                                                            ...
                                                            <Tooltip placement={"top"} title={"Edit medical staff's informations"}>
                                                                <button
                                                                    onClick={() => {
                                                            ...
                                                            <Tooltip placement={"top"} title={"delete medical staff"}>
                                                                <button
                                                                    onClick={() => {
                                ...
                                <div className="flex flex-col items-center justify-center h-[600px]  bg-gradient-to-b from-white to-teal-50   p-8">
                                    ...
                                    <p className="text-gray-600 text-center mb-8 max-w-xl">
                                        {searchTerm
                                            ? "Try adjusting your search terms or browse all medical staff members."
