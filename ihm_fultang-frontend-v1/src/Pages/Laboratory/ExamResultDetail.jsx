...
export function ExamResultDetails() {
  ...
  const handlePrintPDF = () => {
    ...
    doc.setFontSize(14);
    doc.setFont(undefined, "bold");
    doc.text("Informations de l'examen", centerX, yPos, { align: "center" });
    yPos += 8;
    ...
    // Données du tableau
    const examDetails = [
      { label: "Date de l'examen", value: formatDateOnly(examResult.addDate) },
      { label: "Heure", value: formatDateToTime(examResult.addDate) },
      { label: "Patient", value: patientName },
      { label: "Personnel médical", value: staffName },
    ];
    ...
    doc.setTextColor(255, 255, 255);
    doc.text("Notes de l'examen", margin + 5, yPos + 7);
    yPos += 10;

    doc.setTextColor(0, 0, 0);
    doc.setFont(undefined, "normal");
    doc.setFontSize(10);
    const notes = examResult.notes || "Aucune note fournie.";
    const splitNotes = doc.splitTextToSize(notes, tableWidth - 10);
    const notesHeight = Math.max(splitNotes.length * 5 + 10, 20);
    doc.rect(margin, yPos, tableWidth, notesHeight);
    doc.text(splitNotes, margin + 5, yPos + 7);
    yPos += notesHeight + 5;

    // Fichier joint
    if (examResult.examFile) {
      doc.setFontSize(12);
      doc.setFont(undefined, "bold");
      ...
      doc.text(
        `📎 Fichier disponible (voir lien dans le document original)`,
        margin + 5,
        yPos + 8,
      );
    }
    ...
    // Sauvegarder le PDF
    doc.save(
      `Resultat_Examen_${patientName.replace(/\s+/g, "_")}_${formatDateOnly(examResult.addDate)}.pdf`,
    );
  };
  ...
  if (error) {
    return (
      <ServerErrorPage errorStatus={error.status} message={error.message} />
    );
  }
    ...
    <LaboratoryDashBoard linkList={laboratoryNavLink} requiredRole={"Labtech"}>
      ...
      <div className="flex flex-col min-h-screen p-8">
        ...
        <EditExamResult
          ...
          refreshExamResult={refreshExamResult}
        />
      </div>
    </LaboratoryDashBoard>
  );
...
function EditExamResult({ examResult, refreshExamResult }) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      formData.append(
        "idExamRequest",
        examResult.idExamRequest?.id || examResult.idExamRequest || "",
      );
      formData.append(
        "idMedicalFolderPage",
        examResult.idMedicalFolderPage?.id ||
          examResult.idMedicalFolderPage ||
          "",
      );
      formData.append(
        "idPatient",
        examResult.idPatient?.id || examResult.idPatient || "",
      );
      formData.append(
        "idMedicalStaff",
        examResult.idMedicalStaff?.id || examResult.idMedicalStaff || "",
      );
      ...
      } else if (examResult.examFile) {
        formData.append("examFile", examResult.examFile);
      }

      const response = await axiosInstance.put(
        `/exam-result/${examResult.id}/`,
        formData,
        {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      ...
      {updateSuccess && (
        <p className="text-green-600 mb-4">Exam result updated successfully.</p>
      )}
