    ...
    async function fetchStats() {
      try {
        ...
        if (response.status === 200) {
          ...
          // Filtrer les examens non complétés
          const pendingExams = examRequestsArray.filter(
            (exam) => exam.examStatus !== "Completed",
          );

          // Calculate unique patient IDs from pending exam requests.
          const uniquePatientIds = new Set(
            pendingExams.map((exam) => exam.idPatient?.id).filter(Boolean),
          );

          setStats({
            patients: uniquePatientIds.size,
            examRequests: pendingExams.length,
          });
        }
