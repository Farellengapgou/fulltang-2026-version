...
export  function AppointmentList() {
    ...
    async function retrieveDentistAppointments(dentistId)
    {
        setIsLoading(true);
        try
        {
            const response  = await axiosInstance.get(`/appointment/dentist/${dentistId}/`);
            setIsLoading(false);
    ...
    useEffect(() => {
        if(userData.id)
        {
            retrieveDentistAppointments(userData.id);
        }
    }, [userData.id]);
