    ...
    const fetchArticles = async () => {
      try {
        ...
        if (selectedWarehouseId) {
          ...
          const stockLevelsRes = await getStockLevels({
        ...
        if (
          selectedArticleId &&
          !articlesRes.results.some((a) => a.id === parseInt(selectedArticleId))
        ) {
        ...
        {isLoading ? (
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-8">
                ...
                <tbody>
                  {stockCardData.movements.map((movement, idx) => (
                    <tr key={idx} className="hover:bg-gray-50">
                      ...
                      <td className="border border-gray-300 px-2 py-2 text-right">
                        {movement.entry_price
                          ? formatCurrency(movement.entry_price)
                          : "-"}
                      </td>
                      <td className="border border-gray-300 px-2 py-2 text-right">
                        {movement.entry_value
                          ? formatCurrency(movement.entry_value)
                          : "-"}
                      </td>
                      <td className="border border-gray-300 px-2 py-2 text-right">
                        {movement.exit_qty}
                      ...
                      <td className="border border-gray-300 px-2 py-2 text-right">
                        {movement.exit_price
                          ? formatCurrency(movement.exit_price)
                          : "-"}
                      </td>
                      <td className="border border-gray-300 px-2 py-2 text-right">
                        {movement.exit_value
                          ? formatCurrency(movement.exit_value)
                          : "-"}
                      </td>
            ...
            <div className="mt-6 pt-4 border-t border-gray-300">
              <p className="text-xs text-gray-600">
                <strong>Note:</strong> Fiche de stock établie conformément à
                l'Acte Uniforme OHADA relatif au droit comptable et à
                l'information financière (Articles 17-18). Méthode de
                valorisation: Prix Moyen Pondéré (PMP).
              </p>
            </div>
          </div>
        ) : (
          <div className="bg-gray-50 border-2 border-dashed border-gray-300 rounded-lg p-12 text-center">
            ...
            <p className="text-gray-600">
              Sélectionnez un article et cliquez sur "Générer la fiche" pour
