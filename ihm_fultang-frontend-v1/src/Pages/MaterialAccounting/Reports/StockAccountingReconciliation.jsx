        ...
        <AccountantDashBoard linkList={MaterialAccountingNavLink} requiredRole={"MaterialAccountant"}>
            ...
            <div className="mx-auto p-12">
                <div className="flex justify-between items-center mb-8">
                    ...
                    <button
                        onClick={loadData}
                        className="bg-primary-end text-white px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-opacity-90 transition-all cursor-pointer"
                        disabled={isLoading}
                    >
                        <RefreshCw className={`h-4 w-4 ${isLoading ? 'animate-spin' : ''}`} />
                        {isLoading ? "Traitement..." : "Lancer le rapprochement"}
                    </button>
                </div>
