    ...
    const filterMovements = () => {
        ...
        if (typeFilter) {
            if (typeFilter === "TRANSFER") {
                filtered = filtered.filter((mov) => mov.movement_type === "TRANSFER" || mov.movement_reason === "TRANSFER");
            } else {
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                    ...
                    <div className="bg-white rounded-lg shadow-sm border border-green-200 p-4 bg-green-50">
                        ...
                        <p className="text-2xl font-bold text-green-800">
                            {movements.filter(m => m.movement_type === "IN" && m.movement_reason !== "TRANSFER").length}
                        </p>
                    </div>
                    <div className="bg-white rounded-lg shadow-sm border border-red-200 p-4 bg-red-50">
                        ...
                        <p className="text-2xl font-bold text-red-800">
                            {movements.filter(m => m.movement_type === "OUT" && m.movement_reason !== "TRANSFER").length}
                        </p>
                    </div>
                    <div className="bg-white rounded-lg shadow-sm border border-blue-200 p-4 bg-blue-50">
                        ...
                        <p className="text-2xl font-bold text-blue-800">
                            {movements.filter(m => m.movement_type === "TRANSFER" || m.movement_reason === "TRANSFER").length}
                        </p>
                    </div>
                </div>
                                    ...
                                    <tr key={movement.id}>
                                        ...
                                        <td className="px-6 py-4 bg-gray-50 text-center">
                                            <div className="flex items-center justify-center gap-2">
                                                {movement.movement_reason === "TRANSFER" ? getMovementIcon("TRANSFER") : getMovementIcon(movement.movement_type)}
                                                <span className="text-sm font-medium">
                                                    {movement.movement_reason === "TRANSFER" ? "Transfert" : movementTypes[movement.movement_type]?.label}
                                                </span>
                                            </div>
                                        </td>
