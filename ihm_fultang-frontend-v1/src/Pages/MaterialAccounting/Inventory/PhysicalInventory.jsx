...
export function PhysicalInventory() {
    ...
    const loadData = async () => {
        try {
            ...
            console.log("Inventories loaded:", data.results);
    ...
    const getStatusBadge = (status) => {
        const badges = {
            PLANNED: { label: "Planifié", color: "bg-blue-50 text-blue-700" },
            IN_PROGRESS: { label: "Planifié", color: "bg-blue-100 text-blue-800" },
            COMPLETED: { label: "Terminé", color: "bg-purple-100 text-purple-800" },
            VALIDATED: { label: "Terminé", color: "bg-purple-100 text-purple-800" },
            POSTED: { label: "Terminé", color: "bg-purple-200 text-purple-900" },
            ...
            <div className="mx-auto p-12">
                <div className="flex justify-between items-center mb-6">
                    ...
                    <button
                        ...
                        className="flex items-center px-4 py-2 bg-teal-600 text-white rounded-lg hover:bg-teal-700 transition-all shadow-md"
                    >
                    ...
                    <>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                            ...
                            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                                <p className="text-sm text-blue-600">Planifié</p>
                                <p className="text-2xl font-bold text-blue-800">
                                    {inventories.filter((i) => i.status === "PLANNED" || i.status === "IN_PROGRESS").length}
                                </p>
                            </div>
                            <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
                                <p className="text-sm text-purple-600">Terminé</p>
                                <p className="text-2xl font-bold text-purple-800">
                                    {inventories.filter((i) => i.status === "VALIDATED" || i.status === "COMPLETED" || i.status === "POSTED").length}
                                </p>
                            </div>
                        </div>
                                        ...
                                        <tr key={inventory.id}>
                                            ...
                                            <td className="px-6 py-4 bg-gray-50 rounded-r-xl">
                                                <div className="flex items-center justify-center">
                                                    <button
                                                        onClick={() => navigate(`/material-accounting/inventories/${inventory.id}`)}
