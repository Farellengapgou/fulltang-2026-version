...
export function SupplierModal({ isOpen, onClose, onRefresh, editingSupplier }) {
    ...
    const loadChartOfAccounts = async () => {
        try {
            ...
            setAccounts([
                { id: 12, account_number: '4011', account_name: 'Fournisseurs d\'exploitation' },
                { id: 13, account_number: '4012', account_name: 'Fournisseurs - Groupe' },
            ]);
        }
    };
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6 overflow-y-auto max-h-[75vh]">
                    ...
                    <div className="space-y-2 bg-yellow-50 p-4 rounded-xl border border-yellow-200">
                        <label className="text-sm font-bold text-yellow-800 flex items-center gap-2">
                            <Calculator className="h-4 w-4" /> Compte Comptable (Obligatoire pour les achats)
                        </label>
                        <select
                            value={formData.account}
                            onChange={(e) => setFormData({ ...formData, account: e.target.value })}
                            className="w-full p-3 bg-white border border-yellow-300 rounded-xl focus:ring-2 focus:ring-yellow-500 outline-none"
                        >
                            <option value="">Sélectionner un compte...</option>
                            {accounts.filter(a => a.account_number && a.account_number.toString().startsWith('40')).map(a => (
                        ...
                        <p className="text-xs text-yellow-700 mt-1">
                            Ce compte sera crédité lors de la comptabilisation des bons d'entrée.
                            Généralement un compte de classe 401. {accounts.length === 0 && "(Chargement des comptes...)"}
                        </p>
                    </div>
