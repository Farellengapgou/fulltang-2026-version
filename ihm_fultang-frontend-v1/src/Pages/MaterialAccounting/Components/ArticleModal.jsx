...
export function ArticleModal({ isOpen, onClose, onRefresh, editingArticle }) {
    ...
    const generateArticleCode = () => {
        const category = categories.find(c => c.id === parseInt(formData.category));
        const family = families.find(f => f.id === parseInt(formData.family));

        if (!category) return "";

        // Format: CAT-FAM- or CAT- if no family
        const catCode = category.code || "ART";
        if (family && family.code) {
            return `${catCode}-${family.code}-`;
        }
        return `${catCode}-`;
    };
    ...
    const handleCategoryChange = (e) => {
        ...
        const selectedCategory = categories.find(c => c.id === parseInt(newCategory));

        // Auto-fill accounts from category if creating new article
        const newFormData = {
            ...formData,
        ...
        if (!editingArticle && selectedCategory) {
            // Inherit category's default accounts
            if (selectedCategory.default_stock_account?.id) {
                newFormData.stock_account = selectedCategory.default_stock_account.id;
            }
            if (selectedCategory.default_expense_account?.id) {
                newFormData.purchase_account = selectedCategory.default_expense_account.id;
            }
        }
    ...
    const handleFamilyChange = (e) => {
        ...
        // Auto-generate code if creating new article
        if (!editingArticle && formData.category) {
            const category = categories.find(c => c.id === parseInt(formData.category));
            const family = families.find(f => f.id === parseInt(newFamily));

            let newCode = "";
            if (category) {
                const catCode = category.code || "ART";
                if (family && family.code) {
                    newCode = `${catCode}-${family.code}-`;
                } else {
                    newCode = `${catCode}-`;
                }
            }

            setFormData({ ...formData, family: newFamily, code: newCode });
        } else {
