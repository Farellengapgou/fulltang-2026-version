...
export function GoodsReceiptModal({ isOpen, onClose, onRefresh }) {
    ...
    const handleAddLine = () => {
        setFormData({
            ...formData,
            lines: [...formData.lines, { article: "", quantity: 1, unit_price: 0 }]
        });
    };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
                <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-primary-end text-white">
                    <h2 className="text-xl font-bold italic">Nouveau Bon d'Entrée</h2>
                        ...
                        {formData.lines.map((line, index) => (
                            <div key={index} className="grid grid-cols-12 gap-3 items-end bg-gray-50 p-3 rounded-xl border border-gray-100">
                                ...
                                <div className="col-span-6 md:col-span-3 space-y-1">
                                    <label className="text-[10px] uppercase font-bold text-gray-500">Prix Unitaire (XAF)</label>
                                    <input
                                        type="number"
                                        min="0"
                                        required
                                        value={line.unit_price}
                                        onChange={(e) => handleLineChange(index, 'unit_price', parseInt(e.target.value))}
                                        className="w-full p-2 bg-white border border-gray-200 rounded-lg outline-none"
                                    />
                                </div>
