...
export function TransferModal({ isOpen, onClose, onRefresh }) {
    ...
    useEffect(() => {
        if (formData.from_warehouse) {
            loadArticles(formData.from_warehouse);
        } else {
            setArticles([]);
        }
    ...
    const loadArticles = async (warehouseId) => {
        try {
            setIsLoading(true);
            const aData = await getStockLevels({ warehouse: warehouseId });
                                    ...
                                    >
                                        ...
                                        {articles.map(a => (
                                            <option key={a.id} value={a.article.id}>
                                                {a.article.name} (Dispo: {a.physical_quantity - a.reserved_quantity})
                                            </option>
                                        ))}
                                    </select>
                                </div>
