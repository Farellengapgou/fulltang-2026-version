...
import { EditDrugInfosModal } from "../AdminViews/EditDrugInfosModal.jsx";
import Loader from "../../GlobalComponents/Loader.jsx";
import ServerErrorPage from "../../GlobalComponents/ServerError.jsx";


export function PharmacyMedicationsList() {
    ...
    const [numberOfMedication, setNumberOfMedication] = useState(0);
    ...
    const filteredMedications = drugList.filter((drug) => {
        const matchesSearch =
            searchTerm === "" ||
    ...
    // If filters are applied → use filtered list
    const displayDrugs = (searchTerm || statusFilter !== "All")
        ? filteredMedications
        : drugList;

    // Extract unique statuses
    const uniqueStatuses = ["All", ...new Set(drugList.map(drug => drug.status))];


    function calculateNumberOfSlide() {
        const totalItems = (searchTerm || statusFilter !== "All") ? filteredMedications.length : numberOfMedication;
    ...
    async function fetchDrugData(url = "/product/") {
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                setNumberOfMedication(response.data.count);
        ...
        <CustomDashboard linkList={pharmacyNavLink} requiredRole={"Pharmacist"}>
            ...
            <div className="mt-5 flex flex-col relative">
                ...
                <div className="flex justify-between mb-5">
                    <p className="font-bold text-xl mt-2 ml-5"> List Of Medications </p>
                    <div className="flex mr-5">
                        ...
                        <div className="mr-4 font-bold">
                            ...
                            <select
                                ...
                                className="ml-2 w-40 h-10 text-white bg-gray-300 px-2 border-gray-200 rounded-lg"
                            >
                                ...
                                <tbody>
                                    {displayDrugs.map((drug, index) => (
                                        <tr key={drug.id || index} className="bg-gray-100">
                                            ...
                                            <td className={`p-4 text-md font-bold text-center ${drug.status === "Available"
                                                ? "text-green-500"
