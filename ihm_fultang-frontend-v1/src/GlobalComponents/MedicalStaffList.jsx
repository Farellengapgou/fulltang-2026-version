...
export function MedicalStaffList() {
    ...
    const [allMedicalStaff, setAllMedicalStaff] = useState([]); // Stocker TOUS les membres pour la recherche
    ...
    // Filtrer le personnel médical en fonction du terme de recherche (recherche instantanée)
    const filteredMedicalStaff = useMemo(() => {
        if (!searchTerm.trim()) {
            return medicalStaffList; // Si pas de recherche, afficher la liste de la page actuelle
        }

        // Rechercher dans TOUS les membres chargés
        const searchLower = searchTerm.toLowerCase().trim();
        return allMedicalStaff.filter(person =>
            person.first_name?.toLowerCase().includes(searchLower) ||
            person.last_name?.toLowerCase().includes(searchLower) ||
            person.email?.toLowerCase().includes(searchLower) ||
            person.role?.toLowerCase().includes(searchLower) ||
            person.gender?.toLowerCase().includes(searchLower) ||
            person.cniNumber?.includes(searchTerm)
        );
    }, [searchTerm, medicalStaffList, allMedicalStaff]);


    // Fonction pour charger TOUS les membres du personnel (pour la recherche globale)
    async function fetchAllMedicalStaff() {
        try {
            const response = await axiosInstance.get("/medical-staff/?page_size=1000");
            if (response.status === 200) {
                setAllMedicalStaff(response.data.results || response.data);
            }
        } catch (error) {
            console.log("Erreur lors du chargement de tout le personnel médical:", error);
        }
    }
    ...
    useEffect(() => {
        ...
        fetchAllMedicalStaff(); // Charger tous les membres au démarrage
    }, []);
        ...
        <>
                ...
                <div className="flex mr-5 mt-2">
                    ...
                    {searchTerm && (
                        <span className="ml-3 mt-2 text-sm text-gray-600">
                            {filteredMedicalStaff.length} result(s)
                        </span>
                    )}
                </div>
            </div>
            ...
            {isLoading ? (
                ...
                <>
                        ...
                        <table className="w-full border-separate border-spacing-y-2">
                            <thead>
                                <tr>
                                    ...
                                    <th className="text-center p-3 text-xl font-bold border-r-2 border-gray-200">First Name</th>
                                    <th className="text-center p-3 text-xl font-bold border-r-2 border-gray-200 ">Last Name</th>
                            ...
                            <tbody className="mt-5">
                                {filteredMedicalStaff.map((person, index) => (
                    ...
                    {/* Pagination - Cacher si recherche active */}
                    {!searchTerm && (
                ...
                // Message quand aucun résultat trouvé pour la recherche
                <div className="flex flex-col items-center justify-center h-[500px] p-8">
                    ...
                    <p className="text-gray-600 mb-6 max-w-xl text-md font-medium">
                        No medical staff member matches &quot;{searchTerm}&quot;. Try a different search term.
                    </p>
                    <button
                        onClick={clearSearch}
                ...
                // Message quand pas de personnel médical enregistré
                <div className="flex flex-col items-center justify-center h-[500px] p-8">
