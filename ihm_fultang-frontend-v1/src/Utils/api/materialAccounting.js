...
const ACCOUNTING_API_BASE = import.meta.env.VITE_BACKEND_FULTANG_API_BASE_ACCOUNTANT_URL || 'http://127.0.0.1:8009/api/v1/accounting';
...
export const getChartOfAccounts = async (params = {}) => {
    ...
    const API_BASE = ACCOUNTING_API_BASE;
...
export const getFamilies = async (params = {}) => {
    if (USE_MOCK_DATA) {
        // ... (Mock implementation logic - simplified for now or copy Category logic)
        // For now, let's assume direct backend usage or minimal mock
        let families = getFromStorage(STORAGE_KEYS.FAMILIES, initialCategories.filter(c => c.parent !== null)); // Reusing categories mock usually works if structure same
        // But better to define initialFamilies separately if needed.
        // Given constraint of time, I'll rely on backend mostly or simple mock.
        return { results: families };
    }
    const query = new URLSearchParams(params).toString();
    const response = await apiCall(`/families/${query ? `?${query}` : ''}`, { method: 'GET' });
    return Array.isArray(response) ? { results: response } : response;
};
...
export const deleteFamily = async (id) => {
    ...
    return apiCall(`/families/${id}/`, { method: 'DELETE' });
};
...
export const getStockCard = async (articleId, warehouseId, dateFrom, dateTo) => {
    ...
    const query = new URLSearchParams({
        ...
        ...(warehouseId && { depot_id: warehouseId }),
        ...(dateFrom && { date_from: dateFrom }),
        ...(dateTo && { date_to: dateTo })
    }).toString();
    const response = await apiCall(`/report/stock-card/?${query}`);
    return Array.isArray(response) ? { results: response } : response;
};

export const getPerpetualInventory = async (date, depotId = "") => {
    if (USE_MOCK_DATA) {
        await delay(800);
        return {
    ...
    return apiCall(`/report/perpetual-inventory/?${query}`);
};

export const getABCAnalysis = async (dateFrom = "", dateTo = "") => {
    if (USE_MOCK_DATA) {
        await delay(800);
        return {
            classA: {
    ...
    return apiCall(`/report/consumption-analysis/?${query}`);
};
...
export const getTurnoverRates = async () => {
    ...
    const response = await apiCall('/report/rotation-rate/');
    return Array.isArray(response) ? { results: response } : response;
};
...
export const getReconciliation = async () => {
    ...
    const response = await apiCall('/report/inventory-variances/');
    return Array.isArray(response) ? { results: response } : response;
};
        ...
        for (let i = 0; i < data.lines.length; i++) {
            ...
            await apiCall(`/receipts/${receipt.id}/lines/`, {
                ...
                body: JSON.stringify({
                    ...
                    batch_number: line.batch_number || data.reference || `LOT-${Date.now()}`,
...
export const validateGoodsReceipt = async (id) => {
    if (USE_MOCK_DATA) {
        await delay(800);
        const index = mockGoodsReceipts.findIndex(r => r.id === parseInt(id));
        if (index !== -1) {
            mockGoodsReceipts[index].status = "VALIDATED";
            saveToStorage(STORAGE_KEYS.GOODS_RECEIPTS, mockGoodsReceipts);
            return mockGoodsReceipts[index];
        }
        return null;
    }
    return apiCall(`/receipts/${id}/validate/`, { method: 'POST' });
};
...
export const postGoodsReceipt = async (id) => {
    if (USE_MOCK_DATA) {
        ...
        const index = mockGoodsReceipts.findIndex(r => r.id === parseInt(id));
        if (index !== -1) {
            mockGoodsReceipts[index].status = "POSTED";
            saveToStorage(STORAGE_KEYS.GOODS_RECEIPTS, mockGoodsReceipts);
            return mockGoodsReceipts[index];
        }
        return null;
    }
    return apiCall(`/receipts/${id}/accounting-entry/`, { method: 'POST' });
};

export const deleteGoodsReceipt = async (id) => {
    if (USE_MOCK_DATA) {
        await delay(500);
        mockGoodsReceipts = mockGoodsReceipts.filter(r => r.id !== parseInt(id));
        saveToStorage(STORAGE_KEYS.GOODS_RECEIPTS, mockGoodsReceipts);
        return { success: true };
    }
...
export const getGoodsIssueDetails = async (id) => {
    if (USE_MOCK_DATA) {
        await delay(500);
        const issue = mockGoodsIssues.find(i => i.id === parseInt(id));
        return issue || null;
    }
...
export const createGoodsIssue = async (data) => {
    if (USE_MOCK_DATA) {
        ...
        const newIssue = {
            ...
            status: "DRAFT",
            line_count: data.lines ? data.lines.length : 0,
            total_amount: data.lines ? data.lines.reduce((sum, line) => sum + (line.quantity * 1000), 0) : 0
        };
...
export const updateGoodsIssue = async (id, data) => {
    if (USE_MOCK_DATA) {
        await delay(800);
        const index = mockGoodsIssues.findIndex(i => i.id === parseInt(id));
        if (index !== -1) {
            mockGoodsIssues[index] = { ...mockGoodsIssues[index], ...data };
            saveToStorage(STORAGE_KEYS.GOODS_ISSUES, mockGoodsIssues);
            return mockGoodsIssues[index];
        }
        return null;
    }
    ...
    const currentDetails = await apiCall(`/issues/${id}/`);
    if (currentDetails.lines) {
        for (let line of currentDetails.lines) {
            await apiCall(`/issues/${id}/lines/${line.id}/`, { method: 'DELETE' });
        }
    }
...
export const validateGoodsIssue = async (id) => {
    if (USE_MOCK_DATA) {
        await delay(800);
        const index = mockGoodsIssues.findIndex(i => i.id === parseInt(id));
        if (index !== -1) {
            mockGoodsIssues[index].status = "VALIDATED";
            saveToStorage(STORAGE_KEYS.GOODS_ISSUES, mockGoodsIssues);
            return mockGoodsIssues[index];
        }
        return null;
    }
    return apiCall(`/issues/${id}/validate/`, { method: 'POST' });
};
...
export const confirmGoodsIssue = async (id) => {
    if (USE_MOCK_DATA) {
        ...
        const index = mockGoodsIssues.findIndex(i => i.id === parseInt(id));
        if (index !== -1) {
            mockGoodsIssues[index].status = "CONFIRMED";
            saveToStorage(STORAGE_KEYS.GOODS_ISSUES, mockGoodsIssues);
            return mockGoodsIssues[index];
        }
        return null;
    }
    return apiCall(`/issues/${id}/confirm/`, { method: 'POST' });
};

export const deleteGoodsIssue = async (id) => {
    if (USE_MOCK_DATA) {
        await delay(500);
        mockGoodsIssues = mockGoodsIssues.filter(i => i.id !== parseInt(id));
        saveToStorage(STORAGE_KEYS.GOODS_ISSUES, mockGoodsIssues);
        return { success: true };
    }
    return apiCall(`/issues/${id}/`, { method: 'DELETE' });
};

export const postGoodsIssue = async (id) => {
    if (USE_MOCK_DATA) {
        await delay(800);
        const index = mockGoodsIssues.findIndex(i => i.id === parseInt(id));
        if (index !== -1) {
            mockGoodsIssues[index].status = "POSTED";
            saveToStorage(STORAGE_KEYS.GOODS_ISSUES, mockGoodsIssues);
            return mockGoodsIssues[index];
        }
        return null;
    }
    return apiCall(`/issues/${id}/post/`, { method: 'POST' });
};
...
export const sendTransfer = async (id) => {
    if (USE_MOCK_DATA) {
        await delay(800);
        const index = mockTransfers.findIndex(t => t.id === parseInt(id));
        if (index !== -1) {
            mockTransfers[index].status = "SENT";
            saveToStorage(STORAGE_KEYS.TRANSFERS, mockTransfers);
            return mockTransfers[index];
        }
        return null;
    }
    return apiCall(`/transfers/${id}/send/`, { method: 'POST' });
};
...
export const receiveTransfer = async (id) => {
    if (USE_MOCK_DATA) {
        ...
        const index = mockTransfers.findIndex(t => t.id === parseInt(id));
        if (index !== -1) {
            mockTransfers[index].status = "RECEIVED";
            saveToStorage(STORAGE_KEYS.TRANSFERS, mockTransfers);
            return mockTransfers[index];
        }
        return null;
    }
    return apiCall(`/transfers/${id}/receive/`, { method: 'POST' });
};
...
export const deleteTransfer = async (id) => {
    ...
    return apiCall(`/transfers/${id}/`, { method: 'DELETE' });
};
...
export const createPhysicalInventory = async (data) => {
    if (USE_MOCK_DATA) {
        await delay(800);
        const newItem = { id: Date.now(), ...data, status: 'PLANNED', article_count: 0, variance_value: 0 };
        mockInventories.push(newItem);
        saveToStorage(STORAGE_KEYS.PHYSICAL_INVENTORIES, mockInventories);
        return newItem;
    }
...
export const initializeInventory = async (id) => {
    if (USE_MOCK_DATA) {
        await delay(1000);
        return { success: true };
    }
    return apiCall(`/inventories/${id}/initialize/`, { method: 'POST' });
};

export const startCountingInventory = async (id) => {
    if (USE_MOCK_DATA) {
        await delay(800);
        return { success: true };
    }
    return apiCall(`/inventories/${id}/start-counting/`, { method: 'POST' });
};
...
export const validateInventory = async (id) => {
    ...
    return apiCall(`/inventories/${id}/validate/`, { method: 'POST' });
};

export const getInventoryLines = async (id) => {
    if (USE_MOCK_DATA) {
        await delay(500);
        return { results: [] };
    }
    const response = await apiCall(`/inventories/${id}/lines/`, { method: 'GET' });
    return Array.isArray(response) ? { results: response } : response;
};

export const updateInventoryLine = async (inventoryId, lineId, data) => {
    if (USE_MOCK_DATA) {
        await delay(500);
        return { success: true };
    }
    return apiCall(`/inventories/${inventoryId}/lines/${lineId}/`, {
        method: 'PATCH',
        body: JSON.stringify(data)
    });
};
