...
from polyclinic.models import Consultation, Prescription, PrescriptionDrug, PolyclinicProduct, PolyclinicProductCategory
from authentication.models import MedicalStaff

class Command(BaseCommand):
    help = 'Seeds initial data for the Pharmacy module'

    def handle(self, *args, **options):
        self.stdout.write("Seeding data for Pharmacy...")
        ...
        if BudgetExercise.objects.count() == 0:
            ...
            self.stdout.write(f"Created BudgetExercise: {budget}")
        else:
            budget = BudgetExercise.objects.first()
            self.stdout.write(f"Using existing BudgetExercise: {budget}")
        ...
        if created: self.stdout.write(f"Created Account: {account}")
        ...
        if created: self.stdout.write(f"Created FinancialOperation: {op}")
        ...
        for p_data in products_data:
            ...
            if created: self.stdout.write(f"Created Product: {p}")
        ...
        if consultation and Prescription.objects.count() == 0:
            ...
            self.stdout.write(f"Created Prescription: {prescription} for patient {consultation.idPatient}")
